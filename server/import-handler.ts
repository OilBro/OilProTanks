...
import {
  analyzeExcelWithManus,
  analyzePDFWithManus,
} from "./manus-analyzer.ts";
import {
  analyzeSpreadsheetWithOpenRouter,
  processSpreadsheetWithAI,
} from "./openrouter-analyzer.ts";
import { analyzePDFWithOpenRouter, processPDFWithAI } from "./pdf-analyzer.ts";
import {
  parseLegacyExcelData,
  convertToSystemFormat,
} from "./legacy-import-mapper.ts";
...
export async function handleExcelImport(buffer: Buffer, fileName: string) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "Unable to read Excel file. The file may be corrupted or in an unsupported format.",
      );
    }
  }
  ...
  if (firstSheetData.length > 0 && firstSheetData[0][0] === "Report No") {
    ...
    const { reportData, thicknessMeasurements, repairRecommendations } =
      convertToSystemFormat(legacyData);
  ...
  if (process.env.MANUS_API_KEY) {
    ...
    try {
      ...
      console.log(
        "AI found measurements:",
        aiAnalysis.thicknessMeasurements?.length || 0,
      );
      console.log(
        "AI found checklist items:",
        aiAnalysis.checklistItems?.length || 0,
      );
      usedManus = true;
    } catch (error: any) {
  ...
  if (!usedManus && process.env.OPENROUTER_API_KEY) {
    ...
    try {
      ...
      if (
        ...
        console.error(
          "AUTHENTICATION ERROR: OpenRouter API key may be invalid or expired",
        );
  ...
  if (usedManus && aiAnalysis.confidence > 0.3) {
    ...
    ({ importedData, thicknessMeasurements, checklistItems } =
      await processSpreadsheetWithAI(workbook, aiAnalysis));
  }
  ...
  if (checklistItems.length < 5) {
    ...
    checklistItems = [
      ...checklistItems,
      ...extractChecklistFromWorkbook(workbook),
    ];
  }
  ...
  if (aiAnalysis.confidence < 0.5) {
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      const sheetData = XLSX.utils.sheet_to_json(sheetWorksheet, {
        defval: "",
        raw: false,
      });
      ...
      console.log(
        `Standard parsing sheet "${sheetName}" with ${sheetData.length} rows`,
      );
      ...
      for (const row of sheetData) {
        ...
        for (const [key, value] of Object.entries(rowObj)) {
          if (typeof value === "number" && value > 0.05 && value < 3) {
            ...
            const exists = thicknessMeasurements.some(
              (m) =>
                m.location === location &&
            ...
            if (!exists) {
              const measurement = {
                ...
                originalThickness:
                  rowObj["Original"] || rowObj["Nominal"]
                    ? String(rowObj["Original"] || rowObj["Nominal"])
                    : null,
              ...
              console.log(
                `Found thickness in "${sheetName}": ${measurement.location} = ${measurement.currentThickness}`,
              );
            }
          }
        }
    ...
    if (tankCount > 1) {
      ...
      console.log(
        `Found ${tankCount} potential tanks:`,
        Object.keys(potentialTanks),
      );
      ...
      for (const [tankId, tankInfo] of Object.entries(potentialTanks)) {
        ...
        multiTankImports.push({
          importedData: tankImportedData,
          thicknessMeasurements,
          checklistItems,
        });
      }
        ...
        for (const key of Object.keys(rowObj)) {
          ...
          if (value && !isNaN(parseFloat(value))) {
            ...
            if (numValue > 0.05 && numValue < 3) {
              ...
              if (
                !isThickness &&
                (keyLower.includes("course") || keyLower.match(/^\d+$/))
              ) {
                isThickness = true;
              }
              ...
              if (isThickness) {
                const measurement = {
                  location:
                    findFieldValue(rowObj, locationFields) ||
                    key ||
                    `Point ${thicknessMeasurements.length + 1}`,
                  elevation: rowObj["Elevation"] || rowObj["Course"] || "0",
                  currentThickness: String(numValue),
                  component,
                  measurementType,
                  originalThickness: String(
                    rowObj["Original Thickness"] ||
                      rowObj["Nominal Thickness"] ||
                      "0.375",
                  ),
                  createdAt: new Date().toISOString(),
                };
                ...
                const exists = thicknessMeasurements.some(
                  (m) =>
                    ...
                    Math.abs(
                      parseFloat(m.currentThickness) -
                        parseFloat(measurement.currentThickness),
                    ) < 0.001 &&
                ...
                if (!exists) {
                  ...
                  const originalThicknessNum =
                    typeof measurement.originalThickness === "number"
                      ? measurement.originalThickness
                      : parseFloat(measurement.originalThickness) || 0;
                  const currentThicknessNum =
                    typeof measurement.currentThickness === "number"
                      ? measurement.currentThickness
                      : parseFloat(measurement.currentThickness) || 0;
                  // Estimate age from inspection date if available
                  let ageInYears = 1;
                  if (
                    importedData.inspectionDate &&
                    importedData.lastInspection
                  ) {
                  ...
                  if (
                    measurement.component === "Shell" &&
                    importedData.diameter
                  ) {
                    try {
                      const {
                        calculateMinimumRequiredThickness,
                      } = require("./api653-calculations");
                  ...
                  try {
                    ...
                    const rates = calculateCorrosionRate(
                      originalThicknessNum,
                      currentThicknessNum,
                      ageInYears,
                    );
                  ...
                  try {
                    ...
                    status = determineInspectionStatus(
                      remainingLife,
                      currentThicknessNum,
                      minimumRequired,
                    );
                  ...
                  const measurementWithCalc = {
                    ...
                    thicknessLoss: String(
                      originalThicknessNum - currentThicknessNum,
                    ),
                  ...
                  console.log(
                    `Found thickness measurement: [${measurement.component}] ${measurement.location} = ${measurement.currentThickness} | corrosionRate=${corrosionRate}, remainingLife=${remainingLife}, status=${status}`,
                  );
                }
              }
            }
    ...
    for (const row of data) {
      ...
      for (const pattern of tankIdPatterns) {
        ...
        if (value && String(value).trim() && String(value).trim() !== "") {
          ...
          console.log(
            `Found Tank ID in data: "${extractedTankId}" from field "${pattern}"`,
          );
          break;
        }
      }
  ...
  if (!importedData.service) {
        ...
        if (value && String(value).trim()) {
          ...
          for (const [key, standardValue] of Object.entries(serviceMapping)) {
            if (normalizedValue.includes(key)) {
              ...
              console.log(
                `Detected service type: "${detectedService}" from "${value}"`,
              );
              break;
            }
          }
    ...
    importedData.service =
      serviceMapping[normalizedService] || normalizedService;
  }
  ...
  const parseNumericWithUnits = (value: any): number | null => {
    ...
    const cleaned = String(value)
      .replace(
        /\s*(ft|feet|ft\.|in|inches|in\.|gal|gallons|bbl|barrels|years|yrs|yr)\.?\s*$/i,
        "",
      )
  ...
  if (importedData.originalThickness !== undefined) {
    importedData.originalThickness = parseNumericWithUnits(
      importedData.originalThickness,
    );
  ...
  if (importedData.yearsSinceLastInspection !== undefined) {
    ...
    console.log(
      `Parsed yearsSinceLastInspection: ${importedData.yearsSinceLastInspection}`,
    );
  }
  ...
  if (importedData.specificGravity !== undefined) {
    importedData.specificGravity = parseNumericWithUnits(
      importedData.specificGravity,
    );
  }
  ...
  const processedMeasurements = thicknessMeasurements.map(
    (measurement, index) => {
      ...
      if (processed.originalThickness !== undefined) {
        processed.originalThickness = parseNumericWithUnits(
          processed.originalThickness,
        );
      }
      if (processed.corrosionRate !== undefined) {
        processed.corrosionRate = parseNumericWithUnits(
          processed.corrosionRate,
        );
      }
      if (processed.remainingLife !== undefined) {
        processed.remainingLife = parseNumericWithUnits(
          processed.remainingLife,
        );
      }
  ...
  console.log(
    "Final importedData before return:",
    JSON.stringify(importedData, null, 2),
  );
  console.log(
    `Processed ${processedMeasurements.length} thickness measurements`,
  );
  ...
  return {
    importedData,
    thicknessMeasurements: Array.isArray(processedMeasurements)
...
function extractChecklistFromWorkbook(workbook: XLSX.WorkBook): any[] {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (firstCell) {
        if (firstCell.toLowerCase().includes("external"))
          currentCategory = "External Inspection";
        else if (firstCell.toLowerCase().includes("internal"))
          currentCategory = "Internal Inspection";
        else if (firstCell.toLowerCase().includes("foundation"))
          currentCategory = "Foundation";
        else if (firstCell.toLowerCase().includes("shell"))
          currentCategory = "Shell";
        else if (firstCell.toLowerCase().includes("roof"))
          currentCategory = "Roof";
        else if (
          firstCell.toLowerCase().includes("bottom") ||
          firstCell.toLowerCase().includes("floor")
        )
          currentCategory = "Bottom/Floor";
        else if (firstCell.toLowerCase().includes("nozzle"))
          currentCategory = "Nozzles";
        else if (firstCell.toLowerCase().includes("appurtenance"))
          currentCategory = "Appurtenances";
        else if (firstCell.toLowerCase().includes("containment"))
          currentCategory = "Containment";
        else if (firstCell.toLowerCase().includes("safety"))
          currentCategory = "Safety Equipment";
      }
      ...
      for (const pattern of checklistPatterns) {
        if (pattern.test(rowText)) {
          ...
          for (let j = 0; j < row.length; j++) {
            ...
            for (const [statusKey, statusPattern] of Object.entries(
              statusPatterns,
            )) {
          ...
          if (itemText && itemText.length > 5) {
            ...
            const exists = checklistItems.some(
              (item) => item.item.toLowerCase() === itemText.toLowerCase(),
            );
            ...
            if (!exists) {
              ...
              console.log(
                `Found checklist item: ${currentCategory} - ${itemText} [${status}]`,
              );
            }
          }
    ...
    const standardItems = [
      {
        ...
        item: "Shell external coating condition",
        status: "satisfactory",
      },
      {
        category: "External Inspection",
        item: "Foundation settlement and level",
        status: "satisfactory",
      },
      {
        category: "External Inspection",
        item: "Nozzles and appurtenances",
        status: "satisfactory",
      },
      {
        category: "External Inspection",
        item: "Grounding and cathodic protection",
        status: "satisfactory",
      },
      {
        category: "Shell",
        item: "Shell plate condition and corrosion",
        status: "satisfactory",
      },
      {
        category: "Shell",
        item: "Weld seams and heat affected zones",
        status: "satisfactory",
      },
      {
        category: "Foundation",
        item: "Concrete ringwall condition",
        status: "satisfactory",
      },
      {
        category: "Foundation",
        item: "Chime area sealing",
        status: "satisfactory",
      },
      {
        category: "Containment",
        item: "Secondary containment integrity",
        status: "satisfactory",
      },
      {
        category: "Safety Equipment",
        item: "Pressure/vacuum relief valves",
        status: "satisfactory",
      },
    ];
...
export async function handlePDFImport(buffer: Buffer, fileName: string) {
  ...
  try {
    ...
    if (process.env.MANUS_API_KEY) {
      console.log(
        "Manus API key configured, using Manus AI for PDF analysis...",
      );
      try {
        ...
        pdfAnalysis = {
          ...
          mappingSuggestions: manusResult.mappingSuggestions || {},
          detectedFields: [],
          extractedText: manusResult.extractedText || extractedText || "",
        ...
        console.log(
          "AI found measurements:",
          pdfAnalysis.thicknessMeasurements?.length || 0,
        );
        console.log(
          "AI found checklist items:",
          pdfAnalysis.checklistItems?.length || 0,
        );
        usedManus = true;
      } catch (error: any) {
    ...
    if (usedManus && pdfAnalysis.confidence > 0.3) {
      ...
      ({
        ...
        preview,
        aiAnalysis,
      } = await processPDFWithAI(pdfAnalysis));
    }
    ...
    console.log(
      ...
      importedData.tankId,
      "Type:",
      typeof importedData.tankId,
    );
    console.log(
      "Service:",
      importedData.service,
      "Type:",
      typeof importedData.service,
    );
    console.log(
      "Inspector:",
      importedData.inspector,
      "Type:",
      typeof importedData.inspector,
    );
    ...
    if (aiAnalysis.confidence >= 0.3) {
      ...
      console.log(
        usedManus
          ? "Manus AI successfully analyzed the PDF!"
          : "OpenRouter AI successfully analyzed the PDF!",
      );
    } else {
      console.log("=== AI ANALYSIS FAILED OR LOW CONFIDENCE ===");
      console.log("AI confidence:", aiAnalysis.confidence);
      console.log(
        "AI could not analyze the PDF properly, using basic parsing...",
      );
    }
    ...
    return {
      importedData,
      thicknessMeasurements: Array.isArray(thicknessMeasurements)
        ? thicknessMeasurements
        : [],
      checklistItems: Array.isArray(checklistItems) ? checklistItems : [],
